...
typedef enum AIErrorType {
    AIError_User  = 1, // 用户层错误, 上传图片有问题等
    AIError_Code  = 2, // 代码层错误, OpenCV等
    AIError_Other = 3, // 其它错误
} AIErrorType;

typedef struct AIErrorDetail {
    void *      ai_frame;      // 送入分析的数据
    AIErrorType ai_error_type; // 错误类型
    int         error_code;    // 错误码
    std::string detail;        // 错误描述
    int         reserved[6];   // 保留字段
} AIErrorDetail;
...
typedef struct ShelfPhotoAIFrame {
    ...
    std::string debug_path; /// TODO: 调试目录, 记录分析库运行的一些信息, 由分析库自己管理
                            /// 最多允许100M空间, 日志大小建议1M, 其它文件根据需要最好不要太大
                            /// 后期会提供简单的查询及下载功能
} ShelfPhotoAIFrame;
...
typedef struct AstAIFrame {
    FrameInfo *frame_info;
    std::vector<AstAction> all_expected_action; // 所有期望的行为, 可以是多个
    std::string debug_path; /// TODO: 调试目录, 记录分析库运行的一些信息, 由分析库自己管理
                            /// 最多允许100M空间, 日志大小建议1M, 其它文件根据需要最好不要太大
                            /// 后期会提供简单的查询及下载功能
} AstAIFrame;
...
/// \param camera_type: 应用子类型, SVIP_AI_Start 时传入
/// \param ai_handle: 分析句柄, 一路分析的标识, SVIP_AI_Start 时得到
/// \param ai_error_detail: 错误描述结构体
/// \param user: 用户自定义参数
/// 返回值: 0: 成功, 其它: 参考上方 enum 定义
typedef int (*svip_ai_error_cb)(ApplicationType application_type, CameraType camera_type, void *ai_handle, AIErrorDetail *ai_error_detail, void *user);
...
/// 返回值: 0: 成功, 其它: 参考上方 enum 定义
int SVIP_AI_Start(ApplicationType application_type, CameraType camera_type, void *ai_params, int ai_params_size, svip_ai_result_cb result_cb, void *user, void **ai_handle);
...
int SVIP_AI_ShelfPhoto_Start(ApplicationType application_type, CameraType camera_type, void *ai_params, int ai_params_size, svip_ai_result_cb result_cb, void *user, void **ai_handle);
int SVIP_AI_ShelfPhoto_Stop(void *ai_handle);
int SVIP_AI_ShelfPhoto_SetErrorCB(void *ai_handle, svip_ai_error_cb error_cb, void *user);
...
int SVIP_AI_ShelfFront_Start(ApplicationType application_type, CameraType camera_type, void *ai_params, int ai_params_size, svip_ai_result_cb result_cb, void *user, void **ai_handle);
int SVIP_AI_ShelfFront_Stop(void *ai_handle);
int SVIP_AI_ShelfFront_SetErrorCB(void *ai_handle, svip_ai_error_cb error_cb, void *user);
...
int SVIP_AI_ShelfSide_Start(ApplicationType application_type, CameraType camera_type, void *ai_params, int ai_params_size, svip_ai_result_cb result_cb, void *user, void **ai_handle);
int SVIP_AI_ShelfSide_Stop(void *ai_handle);
int SVIP_AI_ShelfSide_SetErrorCB(void *ai_handle, svip_ai_error_cb error_cb, void *user);
...
int SVIP_AI_ShelfSide_V2_Start(ApplicationType application_type, CameraType camera_type, void *ai_params, int ai_params_size, svip_ai_result_cb result_cb, void *user, void **ai_handle);
int SVIP_AI_ShelfSide_V2_Stop(void *ai_handle);
int SVIP_AI_ShelfSide_V2_SetErrorCB(void *ai_handle, svip_ai_error_cb error_cb, void *user);
...
int SVIP_AI_ShelfFresh_Start(ApplicationType application_type, CameraType camera_type, void *ai_params, int ai_params_size, svip_ai_result_cb result_cb, void *user, void **ai_handle);
int SVIP_AI_ShelfFresh_Stop(void *ai_handle);
int SVIP_AI_ShelfFresh_SetErrorCB(void *ai_handle, svip_ai_error_cb error_cb, void *user);
...
int SVIP_AI_Ast_Start(ApplicationType application_type, CameraType camera_type, void *ai_params, int ai_params_size, svip_ai_result_cb result_cb, void *user, void **ai_handle);
int SVIP_AI_Ast_Stop(void *ai_handle);
int SVIP_AI_Ast_SetErrorCB(void *ai_handle, svip_ai_error_cb error_cb, void *user);
